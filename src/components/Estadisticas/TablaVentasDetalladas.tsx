...
interface TablaVentasDetalladasProps {
  ventas: Venta[];
  cargando: boolean;
  periodo: {
    tipo: string;
...
export default function TablaVentasDetalladas({
  ventas,
  cargando,
  periodo
}: TablaVentasDetalladasProps) {
  ...
  // Función para formatear fecha SIN DESFASE
  ...
  const getMetodoPagoColor = (nombre: string) => {
    const nombreLower = nombre.toLowerCase();
    if (nombreLower.includes('efectivo')) return 'bg-green-100 text-green-800 border border-green-200';
    if (nombreLower.includes('tarjeta')) return 'bg-blue-100 text-blue-800 border border-blue-200';
    if (nombreLower.includes('transferencia')) return 'bg-purple-100 text-purple-800 border border-purple-200';
    if (nombreLower.includes('móvil') || nombreLower.includes('movil')) return 'bg-cyan-100 text-cyan-800 border border-cyan-200';
  ...
  const getLabelPeriodo = () => {
    switch (periodo.tipo) {
      ...
      case 'semana': return 'de esta semana';
      case 'mes': return 'de este mes';
      case 'fecha-especifica': return 'de la fecha seleccionada';
      default: return '';
    }
  };

  // Función segura para obtener subtotal
  const getSubtotal = (prod: VentaProducto, tasaBCV: number) => {
    const esPorPeso = prod.producto?.porPeso && prod.peso;
    
    if (esPorPeso) {
      const pesoNum = parseFloat(prod.peso || '0');
  ...
  // Función segura para mostrar cantidad
  ...
  // Función segura para obtener tipo de producto
  const getTipoProducto = (prod: VentaProducto) => {
    return prod.producto?.porPeso ? "por peso" : "por unidad";
  };

  // Función segura para mostrar precio
  const getPrecioDisplay = (prod: VentaProducto) => {
    const esPorPeso = prod.producto?.porPeso && prod.peso;
    return `$${prod.precioUnitario.toFixed(2)}${esPorPeso ? '/kg' : '/unidad'}`;
  };

  // Función segura para calcular total de productos en una venta
  const getTotalProductosVenta = (productos: VentaProducto[]) => {
    return productos.reduce((sum, prod) => {
      const esPorPeso = prod.producto?.porPeso && prod.peso;
      return esPorPeso ? sum + 1 : sum + prod.cantidad;
    }, 0);
  };
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-12 text-center">
          ...
          <p className="mt-6 text-gray-600 text-lg">Cargando ventas {getLabelPeriodo()}...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">
            No hay ventas {getLabelPeriodo()}
          </h3>
          <p className="text-gray-500 mb-8">
            Aún no se han realizado ventas en este período
          </p>
        </div>
      </div>
    );
  ...
  const totalProductosVendidos = ventas.reduce((sum, venta) =>
    sum + venta.productos.reduce((prodSum, prod) => prodSum + prod.cantidad, 0), 0);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 flex items-center">
              ...
              Ventas {getLabelPeriodo()}
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              {ventas.length} {ventas.length === 1 ? 'venta registrada' : 'ventas registradas'} •
              {totalProductosVendidos} {totalProductosVendidos === 1 ? 'producto vendido' : 'productos vendidos'}
            </p>
          </div>
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="flex items-center gap-4">
              <div className="font-bold text-emerald-600">${totalGeneralUSD.toFixed(2)} USD</div>
              <div className="font-bold text-amber-700">Bs {totalGeneralBs.toFixed(2)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {ventas.map((venta) => {
              const totalProductos = getTotalProductosVenta(venta.productos);
                        ...
                        <div className="bg-white rounded-lg border border-blue-200 p-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div className="bg-amber-50 p-3 rounded-lg">
                              ...
                              <p className="text-xs text-gray-500">Tasa BCV: {venta.tasaBCV.toFixed(2)}</p>
                            </div>
                          </div>
