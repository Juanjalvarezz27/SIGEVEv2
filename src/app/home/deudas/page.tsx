...
import ConfirmModal from "@/src/components/Deudas/ConfirmModal"; // IMPORTAR MODAL NUEVO
...
export default function DeudasPage() {
  ...
  const [modalConfirmOpen, setModalConfirmOpen] = useState(false); // Modal Confirmación
  ...
  // --- LOGICA DEL MODAL DE ELIMINAR ---
  const clickEliminar = (deuda: any) => {
    setDeudaSeleccionada(deuda); // Guardamos cual borrar
    setModalConfirmOpen(true);   // Abrimos modal
  };
  ...
  // ... (Handlers de Create, Update, Abonar se mantienen igual) ...
  ...
  const handleAbonarConfirm = async (monto: number) => {
    try {
        const res = await fetch("/api/deudas", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: deudaSeleccionada.id, abono: monto })
        });
  ...
  // --- FILTROS ESTRICTOS ---
  // Historial ON = Solo PAGADAS
  // Historial OFF = Solo PENDIENTES
  const deudasFiltradas = deudas
    .filter(d => d.tipo === tab)
    .filter(d => mostrarHistorial ? d.estado === "PAGADO" : d.estado === "PENDIENTE");

  const totalPendiente = deudas
    .filter(d => d.tipo === tab && d.estado === "PENDIENTE") // El total siempre muestra la deuda real pendiente
    .reduce((acc, curr) => acc + (curr.monto - curr.abonado), 0);

  const config = tab === "COBRAR" 
    ? { color: "text-emerald-600", bg: "bg-emerald-50", icon: <UserMinus size={24}/>, titulo: "Cuentas por Cobrar" }
    : { color: "text-orange-600", bg: "bg-orange-50", icon: <Truck size={24}/>, titulo: "Cuentas por Pagar" };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto min-h-screen space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        >
            <History size={16}/> {mostrarHistorial ? "Viendo Historial (Pagadas)" : "Ver Historial"}
        </button>
      </div>
        ...
        {loading ? <p className="text-center col-span-full py-10 text-gray-400">Cargando...</p> : 
         deudasFiltradas.length === 0 ? (
            <div className="col-span-full py-16 text-center text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
                ...
                <p>
                    {mostrarHistorial 
                        ? "No tienes deudas pagadas en el historial." 
            ...
            deudasFiltradas.map((d) => (
                <DeudaCard 
                    ...
                    onEliminar={() => clickEliminar(d)} // NUEVO: Usar función intermedia
                />
            ))
         )}
      </div>
      ...
      <ConfirmModal 
        ...
        mensaje="Esta acción borrará la deuda permanentemente. No podrás recuperarla."
      />
    </div>
  );
}