...
import { signIn, getSession } from "next-auth/react"; // Importamos getSession
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        // 2. Si es exitoso, consultamos la sesión para ver el ROL
        ...
        if (session?.user?.rol === "SUPER_ADMIN") {
            router.refresh();
            router.push("/admin"); // Tu dashboard
        } else {
            router.refresh();
            router.push("/home"); // Dashboard de clientes
        }
      }
    } catch (err) {
      setLoading(false);
          ...
          <div className="relative z-10 py-4 bg-black/10 backdrop-blur-[2px]">
            <div className="flex flex-col items-center justify-center text-center space-y-2">
              ...
              <div className="flex flex-wrap justify-center gap-2 w-full px-4">
                <a href="mailto:jjsalvarezz@gmail.com" className="group flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 hover:bg-white/20 transition-all duration-300 border border-white/10">
                  <Mail size={12} className="text-indigo-200 group-hover:text-white" />
                  <span className="text-[10px] md:text-xs font-medium text-indigo-100 group-hover:text-white">jjsalvarezz@gmail.com</span>
                </a>
                <a href="https://wa.me/584129164371" target="_blank" rel="noreferrer" className="group flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 hover:bg-white/20 transition-all duration-300 border border-white/10">
                  <Phone size={12} className="text-indigo-200 group-hover:text-white" />
                  <span className="text-[10px] md:text-xs font-medium text-indigo-100 group-hover:text-white">0412-916-4371</span>
                </a>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                ...
                <input type="email" required placeholder="ejemplo@empresa.com" className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all text-gray-800 bg-gray-50/50 focus:bg-white text-sm" value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                ...
                <input type={showPassword ? "text" : "password"} required placeholder="••••••••" className="w-full pl-9 pr-9 py-2.5 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all text-gray-800 bg-gray-50/50 focus:bg-white text-sm" value={password} onChange={(e) => setPassword(e.target.value)} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-indigo-600 cursor-pointer transition-colors">
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
            </div>

            <button type="submit" disabled={loading} className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-lg shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 transition-all transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none text-md">
              {loading ? <Loader2 className="animate-spin h-4 w-4" /> : <>Ingresar al sistema <ArrowRight className="h-4 w-4" /></>}
            </button>
          </form>
        </div>
